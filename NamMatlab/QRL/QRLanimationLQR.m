...
for k=1:length(t)
Bx(:,k) = [XQR(k); YQR(k); ZQR(k)] + Blength*(lqrsimoutR.signals.values(:,1,k));
By(:,k) = [XQR(k); YQR(k); ZQR(k)] + Blength*(lqrsimoutR.signals.values(:,2,k));
Bz(:,k) = [XQR(k); YQR(k); ZQR(k)] + Blength*(lqrsimoutR.signals.values(:,3,k));

BxQ(:,k) = [XQR(k); YQR(k); ZQR(k)] + l*(lqrsimoutR.signals.values(:,1,k));
ByQ(:,k) = [XQR(k); YQR(k); ZQR(k)] + l*(lqrsimoutR.signals.values(:,2,k));
BzQ(:,k) = [XQR(k); YQR(k); ZQR(k)] + l*(lqrsimoutR.signals.values(:,3,k));
end
...
hQ   = plot3(XQR(1),YQR(1),ZQR(1),'--b','LineWidth',1);
hQd  = plot3(XQd(1),YQd(1),ZQd(1),':m','LineWidth',2);
hQdk = plot3(XQd(1),YQd(1),ZQd(1),'mx','LineWidth',2,'MarkerSize',15);
...
%     [-Bx(1) Bx(1)],[2*YQR(1)-Bx(2) Bx(2)],[2*ZQR(1)-Bx(3) Bx(3)]
    ...
    set(hQ,'xdata',XQR(1:k),'ydata',YQR(1:k),'zdata',ZQR(1:k));
    ...
    set(hQd,'xdata',XQd(1:k),'ydata',YQd(1:k),'zdata',ZQd(1:k));
    set(hQdk,'xdata',XQd(k),'ydata',YQd(k),'zdata',ZQd(k));
