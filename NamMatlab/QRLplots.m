...
Omega = simoutL.signals.values(10:12,:);
Omegad = simOmegad.signals.values(:,:).';
Omegac = simOmegac.signals.values(:,:).';
dOmega = simoutL.signals.values(13:15,:);
...
f = simoutL.signals.values(16,:);
M = simoutL.signals.values(17:19,:);
omegarot = simoutL.signals.values(20:23,:);
fi = simoutL.signals.values(24:27,:);
...
figure('Name','Load Position')
h_suptitle = suptitle('Load Position');
set(h_suptitle,'FontSize',30);
subplot 311
hold on
plot(t,posL(1,:),'Linewidth',2);
% plot(t,xdes(1)*ones(size(posL(1,:))),'r--','Linewidth',2);
ylabel('x_L [m]','FontSize', 20);
set(gca,'FontSize',16);
% h_t=ylabel('x');
% set(h_t, 'FontSize', 20);
h_legend = legend('x_L','x_{L,d}');
set(h_legend,'FontSize',25);
subplot 312
hold on
plot(t,posL(2,:),'Linewidth',2);
% plot(t,xdes(2)*ones(size(posL(2,:))),'r--','Linewidth',2);
ylabel('y_L [m]','FontSize', 20);
set(gca,'FontSize',16);
% h_t=ylabel('y');
% set(h_t, 'FontSize', 20);
h_legend = legend('y_L','y_{L,d}');
set(h_legend,'FontSize',25);
subplot 313
hold on
plot(t,-posL(3,:),'Linewidth',2);
% plot(t,xdes(3)*ones(size(posL(3,:))),'r--','Linewidth',2);
xlabel('Time [s]','FontSize', 20);
ylabel('z_L [m]','FontSize', 20);
set(gca,'FontSize',16);
% h_t=ylabel('z');
% set(h_t, 'FontSize', 20);
h_legend = legend('z_L','z_{L,d}');
set(h_legend,'FontSize',25);
saveas(gcf,strcat(foldername,'QRLPosition.png'));
...
for j=1:3
    for n=1:3
        k=(j-1)*3+n;
        subplot(3,3,k)
        Rplot = reshape(R_L.signals.values(j,n,:),[length(R_L.signals.values),1]);
