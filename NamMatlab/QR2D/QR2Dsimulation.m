...
POSQR = simoutL.signals.values(1:3,:) - qvec.signals.values'*lL;
XQR = POSQR(1,:);
YQR = POSQR(2,:);
ZQR = -POSQR(3,:);
...
Bx = [XQR(1); YQR(1); ZQR(1)]+0.5*Bframe(:,1);
By = [XQR(1); YQR(1); ZQR(1)]+0.5*Bframe(:,2);
Bz = [XQR(1); YQR(1); ZQR(1)]-0.5*Bframe(:,3);
...
Bx(:,k) = Bcog(:,k)+0.5*(R.signals.values(:,1,k));
By(:,k) = Bcog(:,k)+0.5*(R.signals.values(:,2,k));
Bz(:,k) = Bcog(:,k)-0.5*(R.signals.values(:,3,k));
end
